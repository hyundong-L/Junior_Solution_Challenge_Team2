...
const ElectricPictureUpload = () => {
  const [imageData, setImageData] = useState(null);
  const { currentUser } = useAuth();

  const handleImageUpload = async (dataUrl) => {
    const fileRef = await uploadString(ref(storage, `Electric/${currentUser.email}` + " - " + uuidv4()), dataUrl, 'data_url');
...
export default ElectricPictureUpload;